              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  var response = await http.get(
                    Uri.parse(
                        'https://api.danielrum.in/api/v1/vehicles/licenseplates/${widget.license}'),
                    headers: {
                      'x-api-key': apiKey,
                  ...
                  if (response.body.isEmpty) {
                    ...
                    if (decodedResponse['wykladowca']) {
                      ...
                      // debugPrint('STUDENT');
                      var studentId = decodedResponse['numer_albumu'];
                      response = await http.get(
                        Uri.parse(
                            'https://api.danielrum.in/api/v1/students/$studentId'),
                        headers: {
                          'x-api-key': apiKey,
                          'Content-Type': 'application/json',
                        },
                      ...
                      String studentName = decodedResponse['imie'],
                          studentSurname = decodedResponse['nazwisko'];
                      DateTime cardValidity =
                          DateTime.parse(decodedResponse['data_waznosci']);

                      // Check available spaces
                      response = await http.get(
                        Uri.parse(
                            'https://api.danielrum.in/api/v1/parking_spots'),
                        headers: {
                          'x-api-key': apiKey,
                          'Content-Type': 'application/json',
                        },
                      ...
                      if (occupiedPlaces >= parkingLimit) {
                        Navigator.of(context).pushReplacementNamed(
                            DenyScreen.routeName,
                            arguments:
                                AppLocalizations.of(context)!.parking_limit2);
                      } else {
                        // Check validity of student ID card
                        if (cardValidity.difference(DateTime.now()).inDays >=
                            0) {
                          ...
                          response = await http.post(
                              Uri.parse(
                                'https://api.danielrum.in/api/v1/logs/log/entry',
                              ),
                              headers: {
                                'x-api-key': apiKey,
                                'Content-Type': 'application/json',
                          ...
                          Navigator.of(context).pushReplacementNamed(
                              DenyScreen.routeName,
                              arguments:
                                  AppLocalizations.of(context)!.invalid_id);
                        }
                      }
                    }
                  }
